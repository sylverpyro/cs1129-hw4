...
const int BREAK_WIDTH = ID_WIDTH + MAXDISP_TITLE + BUFFER + 
	MAXDISP_ALBUM + BUFFER + MAXDISP_ARTIST + BUFFER + GENRE_WIDTH +
   	RATING_WIDTH;
...
// Preconditions	:None
//
// Postconditions	:Prompts for and stores a new rating for the song
//-------------------------------------------------------------------
...
void Song::addRating()
{
	...
	// Make sure user only inputs * characters and increment rating
	while (input != '\n')
	{
		if (input == '*')
			rating++;
...
// Preconditions	:None
//
// Postconditions	:Prints out the Plain Text versoin of the char 
// 					 genre and returns a bool true if it is a 
// 					 legitmate genre
//-------------------------------------------------------------------
...
// 					 data in the song and returns a bool true/false
// 					 based on if its a valid rating
//-------------------------------------------------------------------
...
// Preconditions	:None
//
// Postconditions	:Returns a boolean as to existance of song data
//-------------------------------------------------------------------
...
bool Library::openInput()
{
	...
	{
		cout << endl;
		cout << "*** musicLibrary.txt COULD NOT BE READ ***" << endl;
		cout << "Please ensure that another program is not" << endl;
		cout << "using it and that it is in the local dir." << endl;
...
//
// Postconditions	:Loads the output file into outStream.  Returns 
//					 a false boolean if the load failed.
//-------------------------------------------------------------------
...
// Preconditions	:None
//
// Postconditions	:Loads a new song into memory based on user input
//-------------------------------------------------------------------
...
// Preconditions	:None
//
// Postconditions	:Removes a user specified song from the library
//-------------------------------------------------------------------
...
void Library::removeSong()
{
	...
	else
	{
		...
		cout << endl << "YOU ARE ABOUT TO DELETE THIS SONG" << endl;
		if ( confirm() == false)
			removeLocation = -1;
	}
...
//
// Postconditions	:Performs a save of the library if requested by
...
{
	char check;
	cout << endl << "Would you like to save before leaving [y/N] : ";
	getNoError(check);
	cout << endl;
	if ( check == 'Y' || check == 'y' )
		mainLibrary.writeLib();
